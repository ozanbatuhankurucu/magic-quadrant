  ...
  return (
    <div ref={chartRef} className="w-full max-w-[720px] aspect-[640/580]">
      ...
      >
        ...
        <rect x={padding.left} y={padding.top} width={areaWidth / 2} height={areaHeight / 2}
          className="fill-quadrant-challengers dark:fill-quadrant-challengers-dark" opacity={0.5} />
        <rect x={midX} y={padding.top} width={areaWidth / 2} height={areaHeight / 2}
          className="fill-quadrant-leaders dark:fill-quadrant-leaders-dark" opacity={0.5} />
        <rect x={padding.left} y={midY} width={areaWidth / 2} height={areaHeight / 2}
          className="fill-quadrant-niche dark:fill-quadrant-niche-dark" opacity={0.5} />
        <rect x={midX} y={midY} width={areaWidth / 2} height={areaHeight / 2}
          className="fill-quadrant-visionaries dark:fill-quadrant-visionaries-dark" opacity={0.5} />
        ...
        {showGrid && Array.from({ length: 9 }, (_, i) => {
          ...
          return (
            <g key={i}>
              <line x1={gx} y1={padding.top} x2={gx} y2={bottom}
                className="stroke-gray-300 dark:stroke-gray-600" strokeWidth={0.5} strokeDasharray="4 4" />
              <line x1={padding.left} y1={gy} x2={right} y2={gy}
                className="stroke-gray-300 dark:stroke-gray-600" strokeWidth={0.5} strokeDasharray="4 4" />
            </g>
          );
        })}
        ...
        {/* Center axes */}
        <line x1={midX} y1={padding.top} x2={midX} y2={bottom}
          className="stroke-gray-400 dark:stroke-gray-500" strokeWidth={1.5} />
        <line x1={padding.left} y1={midY} x2={right} y2={midY}
          className="stroke-gray-400 dark:stroke-gray-500" strokeWidth={1.5} />

        {/* Border */}
        <rect x={padding.left} y={padding.top} width={areaWidth} height={areaHeight}
          fill="none" className="stroke-gray-400 dark:stroke-gray-500" strokeWidth={1.5} />

        {/* Quadrant labels */}
        <text x={padding.left + areaWidth * 0.25} y={padding.top + areaHeight * 0.12}
          textAnchor="middle" className="fill-gray-400 dark:fill-gray-500" fontSize={14} fontWeight={600}>
          {config.topLeft}
        </text>
        <text x={padding.left + areaWidth * 0.75} y={padding.top + areaHeight * 0.12}
          textAnchor="middle" className="fill-gray-400 dark:fill-gray-500" fontSize={14} fontWeight={600}>
          {config.topRight}
        </text>
        <text x={padding.left + areaWidth * 0.25} y={padding.top + areaHeight * 0.92}
          textAnchor="middle" className="fill-gray-400 dark:fill-gray-500" fontSize={14} fontWeight={600}>
          {config.bottomLeft}
        </text>
        <text x={padding.left + areaWidth * 0.75} y={padding.top + areaHeight * 0.92}
          textAnchor="middle" className="fill-gray-400 dark:fill-gray-500" fontSize={14} fontWeight={600}>
          {config.bottomRight}
        </text>

        {/* Axis labels */}
        <text x={midX} y={viewBoxHeight - 8}
          textAnchor="middle" className="fill-gray-600 dark:fill-gray-300" fontSize={13} fontWeight={500}>
          {config.xAxis} →
        </text>
        <text
        ...
        {[0, 25, 50, 75, 100].map((v) => (
          <g key={`tick-${v}`}>
            <text x={toSvgX(v)} y={bottom + 18}
              textAnchor="middle" className="fill-gray-400 dark:fill-gray-500" fontSize={10}>
              {v}
            </text>
            <text x={padding.left - 10} y={toSvgY(v) + 4}
              textAnchor="end" className="fill-gray-400 dark:fill-gray-500" fontSize={10}>
              {v}
            </text>
          </g>
        ))}
          ...
          return (
            <g key={point.id} style={{ cursor: isDragging ? "grabbing" : "grab" }}>
              {(isHovered || isDragging) && (
                <circle cx={cx} cy={cy} r={14} fill={point.color} opacity={0.15} />
              )}
              <circle
                cx={cx} cy={cy} r={8}
                fill={point.color}
